package com.ysh.java.basement.map;
...
public class MyHashMap<K, V> {
	...
	private void put(Entity<K, V> newEntity, Entity<K, V>[] entities, int capacity) {
		...
		if (entity == null) {
			...
			if (newEntity.getKey().equals(entity.getKey())) {// Find the same
																// key for the
																// first entity,
																// if find then
																// replace the
				...
				while (entity.getNext() != null) {// Find the same key for all
													// the next entity, if find
